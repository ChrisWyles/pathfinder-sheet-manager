...
| Spheres of Power community wiki (`spheresofpower.wikidot.com`) | Scraped by `scripts/scrape-spheres.ts` into `data/spheres/*.json` and `scripts/scrape-sphere-classes.ts` into `data/sphere-classes/*.json` | Fan-maintained reproduction of Open Game Content from Spheres of Power / Might / Guile and the Spherecaster / Practitioner / Champion classes; robots.txt permits crawling. Product Identity is not present on these pages and is not scraped. |
...
Champions of the Spheres. © 2018, Drop Dead Studios; Author: Adam Meyers.

The Practitioner's Handbook. © Drop Dead Studios; Author: Adam Meyers.
